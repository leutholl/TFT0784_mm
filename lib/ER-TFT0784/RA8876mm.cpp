...
FLASHMEM RA8876mm::RA8876mm(int8_t dc, int8_t cs, int8_t rst) 
{
...
FLASHMEM void RA8876mm::begin(uint8_t buad_div) 
{
...
FLASHMEM uint8_t RA8876mm::setBitDepth(uint8_t bitDepth)  
{
  uint8_t bd;
...
FLASHMEM uint8_t RA8876mm::getBitDepth()
{
  return _bitDepth;
}

FLASHMEM void RA8876mm::setFrameRate(uint8_t frRate) 
{
 _frameRate = frRate;
...
FLASHMEM uint8_t RA8876mm::getFrameRate()
{
  return _frameRate;
}

FLASHMEM void RA8876mm::setTearingEffect(bool tearingOn)
{
...
FLASHMEM bool RA8876mm::getTearingEffect()
{
  return _bTearingOn;
}

FLASHMEM void RA8876mm::setTearingScanLine(uint16_t scanLine)
{
...
FLASHMEM uint16_t RA8876mm::getTearingScanLine()
{
  return _tearingScanLine;
}

FLASHMEM void RA8876mm::setRotation(uint8_t r) 
{ 
  _rotation = r & 3;
...
FLASHMEM void RA8876mm::invertDisplay(bool invert) 
{
  SglBeatWR_nPrm_8(invert ? ILI9488_INVON : ILI9488_INVOFF,0,0);
}

FLASHMEM void RA8876mm::onCompleteCB(CBF callback)
{
  _callback = callback;
  isCB = true;
}

FASTRUN void RA8876mm::setAddrWindow(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2) 
{
  uint8_t Command;
...
FASTRUN void RA8876mm::displayInfo(){
  CSLow();
...
FASTRUN void RA8876mm::pushPixels16bit(const uint16_t * pcolors, uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2)
{
...
FASTRUN void RA8876mm::pushPixels16bitDMA(const uint16_t * pcolors, uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2){
  while(WR_DMATransferDone == false)
  {
    //Wait for any DMA transfers to complete
  }
...
//Private functions
///////////////////
FLASHMEM void RA8876mm::displayInit() 
{
  uint8_t Command;
...
FASTRUN void RA8876mm::FlexIO_Config_SnglBeat_Read()
{
...
FASTRUN uint8_t RA8876mm::readCommand(uint8_t const cmd){
  while(WR_DMATransferDone == false)
  {
    //Wait for any DMA transfers to complete
  }
...
FASTRUN void RA8876mm::FlexIO_Config_SnglBeat()
{
...
FASTRUN void RA8876mm::FlexIO_Clear_Config_SnglBeat(){
    p->CTRL &= ~FLEXIO_CTRL_FLEXEN;
    p->CTRL |= FLEXIO_CTRL_SWRST;
    p->CTRL &= ~FLEXIO_CTRL_SWRST;
...
FASTRUN void RA8876mm::FlexIO_Config_MultiBeat()
{
    uint32_t i;
...
FASTRUN void RA8876mm::SglBeatWR_nPrm_8(uint32_t const cmd, const uint8_t *value = NULL, uint32_t const length = 0)
{
  while(WR_DMATransferDone == false)
  {
    //Wait for any DMA transfers to complete
...
FASTRUN void RA8876mm::SglBeatWR_nPrm_16(uint32_t const cmd, const uint16_t *value, uint32_t const length)
{
 while(WR_DMATransferDone == false)
  {
    //Wait for any DMA transfers to complete
...
RA8876mm * RA8876mm::dmaCallback = nullptr;
DMAChannel RA8876mm::flexDma;

FASTRUN void RA8876mm::MulBeatWR_nPrm_DMA(uint32_t const cmd,  const void *value, uint32_t const length) 
{
  while(WR_DMATransferDone == false)
  {
    //Wait for any DMA transfers to complete
...
FASTRUN void RA8876mm::_onCompleteCB()
{
if (_callback){
      _callback();
    }
...
FASTRUN void RA8876mm::flexDma_Callback()
{
