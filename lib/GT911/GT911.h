...
class TS_Point {
 public:
  TS_Point(void);
  TS_Point(int16_t x, int16_t y, int16_t z, int8_t id);
  ...
  int8_t id; /* tracking id for multiple touches */
};
...
class GT911 {
  public:
    ...
    // note: GTPoint uint16_t are always little endian - use getPoint() instead
    ...
    GTConfig* readConfig();
    uint8_t writeConfig(GTConfig* cfg);
